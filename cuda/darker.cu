...
// Device code
__global__ void darkGray_kernel(const int width, const int height, const unsigned char * inputImage, unsigned char * darkGrayImage) {
...
void darkGray(const int width, const int height, const unsigned char * inputImage, unsigned char * darkGrayImage) {
	...
	darkGray_kernel<<<numBlocks, threadsPerBlock>>>(width, height, inputImage, darkGrayImage);
